test_that("pbj_pbjBootRobustX performs correctly when: method = 'wild', robust = TRUE, HC3 = TRUE, transform = 'none'", {
  ...
  outdir <- tempfile("dir")
  dir.create(outdir)
  statMap <- lmPBJ(images = pain$data$images, form = ~ splines::ns(x, df = 3), formred = ~ 1, mask = mask,
  ...
  res <- sweep(sqrtSigma$res, 1, rboot(n)/(1-h), '*')
...
# TEST FOR THE LONGITUDINAL MODEL INDEPENDENT #
# test_that("pbj_pbjBootRobustX performs correctly when: method = 'wild', robust = TRUE, HC3 = TRUE, id!=NULL, transform = 'none', W_structure = 'independent'", {
#   # setting up
#   pain <- pain21::pain21()
#   pain$data$group <- factor(sample(1:4, size = nrow(pain$data), replace=TRUE))
#   pain$data$x <- rnorm(nrow(pain$data))
...
# TEST FOR THE LONGITUDINAL MODEL EXCHANGEABLE #
# test_that("pbj_pbjBootRobustX performs correctly when: method = 'wild', robust = TRUE, HC3 = TRUE, id!=NULL, transform = 'none', W_structure = 'exchangeable'", {
#   # setting up
#   set.seed(11)
#   pain <- pain21::pain21()
#   pain$data$group <- factor(sample(1:4, size = nrow(pain$data), replace=TRUE))
